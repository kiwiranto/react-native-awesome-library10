...
class CSurvey extends Component {
   ...
   _renderRating = () => {
      ...
      try {
         const ratingScale = surveyData.surveyQuestions.filter(item => { return item.answer_type == 'rating_scale'; })[0];
   ...
   _renderMultipleChoiceFeedback = () => {
      ...
      try {
         const multipleChoice = surveyData.surveyQuestions.filter(item => { return item.answer_type == 'multiple_choice'; })[0];
         ...
         this.setState({
            ...
            answerIdFreeText: surveyData.surveyQuestions.filter((item) => { return item.answer_type === 'free_text' })[0].answer_id
         });
      } catch (error) {
         this.setState({
            multipleChoiceFeedback: []
   ...
   _handleSelectRating = (selectedRating) => {
      ...
      const answerId = surveyData.surveyQuestions.filter(item => { return item.answer_type == 'rating_scale'; })[0].answer_id;
   ...
   _handleSelectMultipleChoiceFeedback = (itemSelected) => {
      const { surveyData } = this.props;
      const answerId = surveyData.surveyQuestions.filter(item => { return item.answer_type == 'multiple_choice' })[0].answer_id;
   ...
   _renderContent = () => {
      const { surveyVisible, title, subtitle, surveyImage, surveyButtonText, language, t, onClose, setSurveyDashboardDataDispatch } = this.props;
      ...
      if (isSuccess) {
         return (
            <CSurveyPopup
               ...
               handleButtonOnPress={() => {
                  onClose();
                  setSurveyDashboardDataDispatch(false);
               }}
   ...
   render() {
      const { onClose, surveyVisible, setSurveyDashboardDataDispatch } = this.props;
...
const mapDispatchToProps = dispatch => ({
   setSurveyDashboardDataDispatch: data => dispatch({
      type: SET_SURVEY_DASHBOARD,
      isShowDashboard: data
   })
});

export default connect(mapStateToProps, mapDispatchToProps)(CSurvey);